...
if(isset($_POST['save'])){
    $userId=$_SESSION['userid'];
                ...
                $idPro=$_POST['idPro']; 
                $ratedIndex++;
                $sql2="SELECT * from reviewmen where userId=? and idPro=?";
                $stmt2=$pdo->prepare($sql2);
                $stmt2->execute([$userId,$idPro]);
                $nbre=$stmt2->rowCount();
                if($nbre==0){
                    $sql3="INSERT INTO reviewmen(idPro,userId,rateIndex) values(?,?,?)";
                    $stmt3=$pdo->prepare($sql3);
                    $stmt3->execute([$idPro,$userId,$ratedIndex]);

                }else{
                     $sql="UPDATE reviewmen set rateIndex=? where idPro=?";
...
$idPro=$_GET['idMen'];
$sql4="SELECT id from reviewmen where idPro=?";
$stmt4=$pdo->prepare($sql4);
$stmt4->execute([$idPro]);
$nbrID=$stmt4->rowCount();
$sql5="SELECT sum(rateIndex) as 'total'from reviewmen where idPro=?";
...
<body>
    ...
    <div class="container-fluid" style="color:white;height: 100vh;display:flex;align-items:center;justify-content:center;background-color:#34568B;flex-direction:column">
    ...
    <img src="uploads/' . $data['image'] . '" alt="" class="thumbnail">
        ...
        while ($data = $stmt->fetch(PDO::FETCH_ASSOC)) {
            echo '
            <div class = "shadow p-3 mb-5 bg-white rounded">
    <div class="container-fluid" style="height: 100vh;display:flex;align-items:center;justify-content:center;background-color:grey;flex-direction:column">
    ...
    <script>
        var avg=0;
        var ratedIndex=-1;
        var idPro=<?php  if(isset($_GET['idMen'])){
       echo $_GET['idMen']; }else{echo 0;}?>;
        ...
        $(document).ready(function(){
            ...
            $idPro=$_GET['idMen'];
            $userId=$_SESSION['userid'];
            $sql="SELECT rateIndex from reviewmen where idPro=? and userId=?";
            $stmt=$pdo->prepare($sql);
